...
router.get('/data/export/excel/:token', async (req, res) => {
    try {
        ...
        let brand = await Brand.find({}).populate('subcategory', ['name']).populate('category', ['name']).populate('branch', ['name'])
        ...
        const styles = {
          ...
          const specification = {
            _id: { // <- the key should match the actual data key
              displayName: 'Brand ID', // <- Here you specify the column header
          ...
          const report = excel.buildExport(
            [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
              {
                name: 'All Brand Information', // <- Specify sheet name (optional)
          ...
          res.attachment('brands.xlsx'); // This is sails.js specific (in general you need to set headers)
