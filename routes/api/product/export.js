...
router.get('/data/export/excel/:token', async (req, res) => {
    try {
      ...
      if(category){
        ...
        let product = await Product.find(condition).select('name barcode quantity price.sell price.purchase reorderLevel discount vat description weight newProduct specialOffer bestSell branch category subcategory brand supplier sold thumbnail images availableSize').populate('category', ['name']).populate('subcategory', ['name']).populate('brand', ['name']).populate('supplier', ['name']).populate('branch', ['name']).populate('unitType', ['name', 'shortform'])
        ...
        const styles = {
          ...
          let productArrayLoop = product.map((productInfo, index)=>{
            if(productInfo.supplier && productInfo.branch){
              updatedProductListData.push({
                ...
                subcategory: productInfo.subcategory !=null ? productInfo.subcategory.name : '',
                ...
                unitType: productInfo.unitType != null ? productInfo.unitType.shortform : ''
              })
            }  
          })
          ...
          const report = excel.buildExport(
            [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
              {
                name: 'All Product Information', // <- Specify sheet name (optional)
          ...
          res.attachment('products-'+branchInfo.name+'('+categoryInfo.name+').xlsx'); // This is sails.js specific (in general you need to set headers)
