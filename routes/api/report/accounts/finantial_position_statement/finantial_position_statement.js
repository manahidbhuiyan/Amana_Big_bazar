...
// @route GET /accounts/finantial_position_statement//pdf?from=?&to=?
// @description Get finantial position statement report
// @access Private
router.post('/accounts/finantial_position_statement', adminAuth, async (req, res) => {
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "finantial_position_statement.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                ...
                liabilities,
                equities,
                totalAssets: totalAssets.toLocaleString('en-In', {minimumFractionDigits: 2, maximumFractionDigits: 2}),
                totalEquityLiabilities: totalEquityLiabilities.toLocaleString('en-In', {minimumFractionDigits: 2, maximumFractionDigits: 2})
            }

            

            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..','..','reports', 'accounts', 'finantial_position_statement.html'), 'utf8');
            ...
            var document = {
                ...
                path: "./public/reports/statement_of_finantial_position.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/statement_of_finantial_position.pdf',
                        filename: 'statement_of_finantial_position'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /accounts/finantial_position_statement/excel?from=?&to=?
// description Get finantial position statement Excel report
// @access Private
router.get('/accounts/finantial_position_statement/:adminID/:from/:to', async (req, res) => {
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Statement of Finantial Position report date wise from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Statement of Finantial Position report date wise from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('statement_of_finantial_position_date_wise.xlsx'); // This is sails.js specific (in general you need to set headers)
