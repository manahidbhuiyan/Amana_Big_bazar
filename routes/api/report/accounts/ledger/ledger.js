...
// @access Private
router.post('/accounts/ledger/details', adminAuth, async (req, res) => {
    try {
        ...
        let {from, to, group, subgroup, category, subcategory, supplier, cost_center} = req.body
        let type = 'pdf'
        let condition = {}
        ...
        const child = fork(path.join(__dirname, "subprocess", "ledger_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                ...
                data: allDataItems,
                grandTotalDebit: grandTotalDebit.toLocaleString('en-In', {minimumFractionDigits: 1, maximumFractionDigits: 1}),
                grandTotalCredit: grandTotalCredit.toLocaleString('en-In', {minimumFractionDigits: 1, maximumFractionDigits: 1})
            }

            

            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..','..','reports', 'accounts', 'ledger_report_details.html'), 'utf8');
            ...
            var document = {
                ...
                path: "./public/reports/ledger_report_details_date_wise.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/ledger_report_details_date_wise.pdf',
                        filename: 'ledger_report_details_date_wise'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /accounts/ledger/details/excel?from=?&to=?&branch=?&group=?&subgroup=?&category=?&subcategory=?
// description Get disposal details Excel report
// @access Private
router.get('/accounts/ledger/details/:adminID/:from/:to/:group/:subgroup/:category/:subcategory', async (req, res) => {
    try {
        ...
        let cost_center = req.params.cost_center
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                ...
                narration: { // <- the key should match the actual data key
                    displayName: 'Naration', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200 // <- width in pixels
                },
                voucherNo: { // <- the key should match the actual data key
                    displayName: 'Voucher No', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200 // <- width in pixels
                },
                voucher: { // <- the key should match the actual data key
                    displayName: 'Voucher', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200 // <- width in pixels
                },
            ...
            const heading = [
                [{ value: 'Details Ledger report date wise from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
                ...
                [{ value: 'Particular: ' + reportFullDataset.particular, style: styles.shop_info }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Details Ledger report date wise from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('details_ledger_report_date_wise.xlsx'); // This is sails.js specific (in general you need to set headers)
