...
process.on('message', async (msg) => {
    ...
    firstDayFrom = new Date(firstDayFrom)
    firstDayTo = new Date(firstDayTo)
    secondDayFrom = new Date(secondDayFrom)
    secondDayTo = new Date(secondDayTo)
    ...
    let orderListItemsPrevMonth = await OrderForPos.find({
        branch: branch,
        create: {
            $gte: isodate(firstDayFrom),
            $lte: isodate(firstDayTo)
        }
    })
    ...
    let orderListItemsCurrentMonth = await OrderForPos.find({
        branch: branch,
        create: {
            $gte: isodate(secondDayFrom),
            $lte: isodate(secondDayTo)
        }
    })
