...
// @route GET /api/report/category/discount/details?branch=..........
// @description Get category wise discount details report pdf
// @access Private
router.post('/category/discount/details', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "category_discount_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'discount', 'category_wise_discount_report_details.html'), 'utf8');
                ...
                header: {
                    ...
                    contents: {
                        ...
                        <tr>
                            ...
                            <th style="font-size: 10px; text-align: left; width: 35%;">Product Name</th>
            ...
            var document = {
                ...
                path: "./public/reports/category_wise_product_discount_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/category_wise_product_discount_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/category/discount/details/:adminID/:from/:to/:category?branch=..........
// @description Get category wise discount details report excel
// @access Private
router.get('/category/discount/details/:adminID/:from/:to/:category', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Category Wise Discount Report Details From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Category Wise Discount Report Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('category_wise_discount_report_details.xlsx'); // This is sails.js specific (in general you need to set headers)
