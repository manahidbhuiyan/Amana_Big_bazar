...
// @route GET /api/report/payment_method/discount/details/report?branch=..........
// @description Get payment method wise discount report details pdf
// @access Private
router.post('/payment_method/discount/details/report', [
    adminAuth,
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
        check('card', 'Card is required').not().isEmpty()
    ]
], async (req, res) => {
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "payment_method_discount_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "detail payment method wise discount report",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'discount', 'payment_method_wise_discount_details.html'), 'utf8');
                ...
                header: {
                    ...
                    contents: {
                        ...
                        <tr>
                            ...
                            <th style="font-size: 7px; text-align: left; width: 10%;">Card</th>
                            <th style="font-size: 7px; text-align: right; width: 30%">Order Date</th>
            ...
            var document = {
                ...
                path: "./public/reports/payment_method_wise_discount_report_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/payment_method_wise_discount_report_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/payment_method/discount/details/report/:adminID/:from/:to/:card?branch=..........
// @description Get payment method wise discount report details excel
// @access Private
router.get('/payment_method/discount/details/report/:adminID/:from/:to/:card', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                ...
                cardPayment: { // <- the key should match the actual data key
                    displayName: 'Card Amount', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200 // <- width in pixels
                },
                cardDiscount: { // <- the key should match the actual data key
            ...
            const heading = [
                [{ value: 'Payment Method Discount Report Details From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Payment Method Discount Report Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('payment_method_discount_report_details.xlsx'); // This is sails.js specific (in general you need to set headers)
