...
// @route GET /api/report/payment_method/discount/summery/report?branch=..........
// @description Get payment method wise discount report summery pdf
// @access Private
router.post('/payment_method/discount/summery/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "payment_method_discount_summery.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: " Payment Method wise discount report Summary",
                ...
                grandTotalDiscountAmount: grandTotalDiscountAmount.toFixed(2),
                grandTotalAmount: grandTotalAmount.toFixed(2)
            }

            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'discount', 'payment_method_wise_discount_summary.html'), 'utf8');
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 7px; text-align: left; width: 50%; padding-left: 3px">Card</th>
                            <th style="font-size: 7px; text-align: right;">Card Amount</th>
            ...
            var document = {
                ...
                path: "./public/reports/payment_method_wise_discount_report_summmery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/payment_method_wise_discount_report_summmery.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/payment_method/discount/summery/report/:adminID/:from/:to/:card?branch=..........
// @description Get payment method wise discount report summery excel
// @access Private
router.get('/payment_method/discount/summery/report/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                cardName: { // <- the key should match the actual data key
                    displayName: 'Card Name', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 300 // <- width in pixels
                },
                totalAmount: { // <- the key should match the actual data key
                    displayName: 'Card Amount', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200 // <- width in pixels
                },
                totalDiscountAmount: { // <- the key should match the actual data key
            ...
            const heading = [
                [{ value: 'Payment Method Discount Report Summary From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
                ...
                [''],
                ['Grand Total:',
                    reportFullDataset.grandTotalAmount,
                    reportFullDataset.grandTotalDiscountAmount
                ],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Payment Method Discount Report Summary From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('payment_method_discount_report_summary.xlsx'); // This is sails.js specific (in general you need to set headers)
