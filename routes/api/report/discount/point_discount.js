...
// @route GET /api/report/point/discount/report?branch=..........
// @description Get used point wise discount report pdf
// @access Private
router.post('/point/discount/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "point_discount.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "Used Point wise discount report",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'discount', 'point_wise_discount.html'), 'utf8');
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 7px; text-align: left; width: 42%;">User Name</th>
                            <th style="font-size: 7px; text-align: left;">Phone No</th>
                            <th style="font-size: 7px; text-align: center;">Used Point</th>
...
// @route GET /api/report/point/discount/report/:adminID/:from/:to?branch=..........
// @description Get used point wise discount report excel
// @access Private
router.get('/point/discount/report/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                ...
                customerPhone: { // <- the key should match the actual data key
                    displayName: 'Phone No', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 300 // <- width in pixels
                },
                usedPoint: { // <- the key should match the actual data key
                    displayName: 'Used Point', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200 // <- width in pixels
                },
                pointDiscount: { // <- the key should match the actual data key
            ...
            const heading = [
                [{ value: 'Point Discount Report From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Point Discount Report From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('point_discount_report.xlsx'); // This is sails.js specific (in general you need to set headers)
