...
// @route GET /api/report/product/discount?branch=..........
// @description Get product wise discount report pdf
// @access Private
router.post('/product/discount', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "product_discount.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'discount', 'product_wise_discount_report.html'), 'utf8');
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 10px; text-align: left; width: 16%; padding-left: 3px">Barcode</th>
                            <th style="font-size: 10px; text-align: left; width: 43%;">Product Name</th>
            ...
            var document = {
                ...
                path: "./public/reports/product_wise_discount_report.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/product_wise_discount_report.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/product/discount/:adminID/:from/:to?branch=..........
// @description Get product wise discount report excel
// @access Private
router.get('/product/discount/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Product Discount Report From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Product Discount Report From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('product_discount_report.xlsx'); // This is sails.js specific (in general you need to set headers)
