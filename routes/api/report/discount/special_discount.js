...
// @route GET /api/report/special/discount/report?branch=..........
// @description Get special discount report pdf
// @access Private
router.post('/special/discount/report', [
    adminAuth,
    [
    ...
    try {
        ...
        to.setMinutes(59);
        to.setSeconds(59);
        const child = fork(path.join(__dirname, "subprocess", "special_discount.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'discount', 'special_discount.html'), 'utf8');
                ...
                header: {
                    ...
                    contents: {
                        ...
                        <tr>
                            ...
                            <th style="font-size: 7px; text-align: right;">Special Discount</th>
                        </tr>
                        </table>`
                    }
                },
            ...
            var document = {
                ...
                path: "./public/reports/special_discount_report.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/special_discount_report.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/special/discount/report/:adminID/:from/:to?branch=..........
// @description Get special discount report excel
// @access Private
router.get('/special/discount/report/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                ...
                specialDiscountAmount: { // <- the key should match the actual data key
                    displayName: 'Special Discount', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200 // <- width in pixels
                }
            }
            ...
            const heading = [
                [{ value: 'Special Discount Report From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Special Discount Report From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('special_discount_report.xlsx'); // This is sails.js specific (in general you need to set headers)
