...
// @route GET /api/report/supplier/discount/summery?branch=..........
// @description Get supplier wise product discount report summery pdf
// @access Private
router.post('/supplier/discount/summery', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "supplier_discount_summary.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'discount', 'supplier_wise_discount_report_summery.html'), 'utf8');
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 8px; text-align: left; width: 16%; padding-left: 3px">Supplier code</th>
                            <th style="font-size: 8px; text-align: left; width: 50%;">Supplier Name</th>
            ...
            var document = {
                ...
                path: "./public/reports/supplier_wise_discount_report_summery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/supplier_wise_discount_report_summery.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/supplier/discount/summery/:adminID/:from/:to/:supplier?branch=..........
// @description Get supplier wise product discount report summery pdf
// @access Private
router.get('/supplier/discount/summery/:adminID/:from/:to/:supplier', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                serial: { // <- the key should match the actual data key
                    displayName: 'Supplier Serial', // <- Here you specify the column header
            ...
            const heading = [
                [{ value: 'Supplier Wise Discount Report Summery From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Supplier Wise Discount Report Summery From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('supplier_wise_discount_report_summery.xlsx'); // This is sails.js specific (in general you need to set headers)
