...
process.on('message', async (msg) => {
    ...
    let productOperationsArray = productOperations.map(async singleOperation => {
        ...
        let productOperationChildArray = singleOperation.products.map(async productInfo => {
            ...
            let productCategory = await Product.findOne({
                _id: productInfo._id,
                branch: branch
            }).populate('category', 'name').select('id expireDate')

            if (productCategory.category !== null) {
                allDataProductsItems.push([
                    productInfo.name,
                    productCategory.category.name,
                    productCategory.expireDate == null ? '-' : (("0" + productCategory.expireDate.getDate()).slice(-2) + ' ' + months[productCategory.expireDate.getMonth()] + ', ' + productCategory.expireDate.getUTCFullYear()),
    ...
    if (type == 'excel') {
        ...
        allDataItems.map((info) => {
            info.products.map((productsInfo, index) => {
                ...
                dataObj.categoryName = productsInfo[1]
