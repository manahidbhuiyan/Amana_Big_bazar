...
router.post('/category/sell/list', [
    ...
    try {
        ...
        child.on('message', async (response)=>{
            ...
            // const myArrayFiltered = branchWiseSell.filter((el) => {
            //     return reportData.some((f) => {
            //         return f.branch_serialNo === el.branch_serialNo
            //     });
            // });
            //

            let reportFullDataset = {
                title : branch == 'all' ? "All Category Wise Sell List(all branch)" : "All Category Wise Sell List(single branch)",
                ...
                createTime,
                // branch_phone : branchWiseSell.branch_phone,
                company_name : config.get('company').full_name,
                company_logo : config.get('company').company_logo,
                // branch_id: branchWiseSell.branch_serialNo,
                branch_name: branch_name,
                // branch_address: branchWiseSell.branch_address,
                branchWiseSell: branchWiseSell,
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 7px; text-align: left; width: 18%;">Department Code Serial</th>
        ...
        child.on('message', async (response)=>{
            ...
            const specification = {
                ...
                name: { // <- the key should match the actual data key
                    ...
                    headerStyle: styles.headerGreen, // <- Header style
