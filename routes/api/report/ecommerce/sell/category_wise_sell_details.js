...
// @route GET api/report/category/sell/details?branch=..........
// @description Get single category wise sell report pdf 
// @access Private
router.post('/category/sell/details', [
    adminAuth,
    [
    ...
    try {
        ...
        to.setMinutes (59);
        to.setSeconds(59);
        const child = fork(path.join(__dirname, "subprocess", "category_wise_sell_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                ...
                // company_address : config.get('company').company_logo,
                branch_id: branch.serialNo,
                branch_name: branch.name,
                branch_address: branch.address,
                category_code: categoriesOfBranch[0]._id,
                category_name: categoriesOfBranch[0].name,
                // supplier_name: categories.name,
                // supplier_serialNo: categoriesOfBranch.serialNo,
                branchWiseSell: branchWiseSellDetails,
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..','..', 'reports','ecommerce', 'sell', 'category_wise_sell_details.html'), 'utf8');
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 7px; text-align: left; width: 13%; padding-left: 3px">Order date</th>
                            <th style="font-size: 7px; text-align: left; width: 13%; padding-left: 3px">Bar Code</th>
                            <th style="font-size: 7px; text-align: left; width: 35%;">Product</th>
            ...
            var document = {
                ...
                path: "./public/reports/category_wise_sell_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/category_wise_sell_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/category/sell/:adminID/:from/:to/:category?branch=..........
// @description Get single category wise sell report excel 
// @access Private
router.get('/category/sell/:adminID/:from/:to/:category', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                datetime: { // <- the key should match the actual data key
                    displayName: 'Order Date and Time', // <- Here you specify the column header
            ...
            const heading = [
                [{ value: 'Category wise Sell Details from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
                ...
                [''],
                [{ value: 'Category Code: ' + reportFullDataset.supplier_serialNo, style: styles.shop_info }],
                [{ value: 'Category Name: ' + reportFullDataset.supplier_name, style: styles.shop_info }],
                [''],
                ['Grand Total:', ' ', ' ', ' ',
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Category wise Sell Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('category_wise_sell_details.xlsx'); // This is sails.js specific (in general you need to set headers)
