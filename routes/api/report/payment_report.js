...
// @route GET api/report/payment-report?branch=..........
// @description Get all the payment report pdf
// @access Private
router.post('/payment-report', [
    adminAuth,
    [
    ...
    try {
        ...
        to.setMinutes (59);
        to.setSeconds (59);
        const child = fork(path.join(__dirname, "subprocess", "payment_report.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                ...
                data: allSellerSellsInfo,
                grandTotal: grandTotal.toFixed(2)
            }

            //return res.json(reportFullDataset)

            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', 'reports', 'payment_report.html'), 'utf8');
            ...
            var document = {
                ...
                path: "./public/reports/payment_report.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/payment_report.pdf'
                    })
                    // res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/payment-report/:adminID/:from/:to?branch=..........
// @description Get all the payment report excel
// @access Private
router.get('/payment-report/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty()
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                ...
                paymentMethod: { // <- the key should match the actual data key
                    displayName: 'Payment Method', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
            ...
            const heading = [
                [{ value: 'Payment Report From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Payment Report from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('payment_report.xlsx'); // This is sails.js specific (in general you need to set headers)
