...
// @route GET api/report/product/performance?branch=..........
// @description Get product wise performance report pdf
// @access Private
router.post('/product/performance', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "product_wise_performance.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "Product wise " + type + " Performance",
                subtitle: type + " " + reqire_quantity + " Items By Product",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'performance', 'product_wise_performance.html'), 'utf8');
            ...
            var document = {
                ...
                path: "./public/reports/product_wise_performance.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/product_wise_performance.pdf',
                        filename: 'product_wise_performance'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/product/performance/:adminID/:from/:to/:reqire_quantity/:type?branch=..........
// @description Get product wise performance report excel
// @access Private
router.get('/product/performance/:adminID/:from/:to/:reqire_quantity/:type', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty()
    ]
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                ...
                percentage_in_supplier_quantinty: { // <- the key should match the actual data key
                    displayName: 'Qty(%) in Total Qty', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 300 // <- width in pixels
                },
                ...
                percentage_in_supplier_cost_amt: { // <- the key should match the actual data key
                    displayName: 'Amt(%) in Total Qty', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 300 // <- width in pixels
                },
            ...
            const heading = [
                [{ value: 'Product wise performance from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Product wise Performance From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('product_wise_performance.xlsx'); // This is sails.js specific (in general you need to set headers)
