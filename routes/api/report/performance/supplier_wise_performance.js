...
// @route GET api/report/supplier/performance?branch=..........
// @description Get supplier wise performance report pdf
// @access Private
router.post('/supplier/performance', [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "supplier_wise_performance.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "Supplier wise " + type + " Performance",
                subtitle: type + " " + reqire_quantity + " Items By Supplier",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'performance', 'supplier_wise_performance.html'), 'utf8');
                ...
                header: {
                    ...
                    contents: {
                        ...
                        <tr>
                            ...
                            <th style="font-size: 7px; text-align: left; width: 23%;">Product</th>
                            <th style="font-size: 7px; text-align: left;">Cost Price</th>
                            <th style="font-size: 7px; text-align: left;">Sales Price</th>
                            <th style="font-size: 7px; text-align: left;">Sold Qty</th>
                            <th style="font-size: 7px; text-align: left;">Qty(%) in Req Qty</th>
                            <th style="font-size: 7px; text-align: left;">Qty(%) in Total</th>
                            <th style="font-size: 7px; text-align: left;">Cost Amt</th>
                            <th style="font-size: 7px; text-align: left;">Sales Amt</th>
                            <th style="font-size: 7px; text-align: left;">Amt(%) in Req Qty</th>
                            <th style="font-size: 7px; text-align: left;">Amt(%) in Total</th>
            ...
            var document = {
                ...
                path: "./public/reports/supplier_wise_performance.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/supplier_wise_performance.pdf',
                        filename: 'supplier_wise_performance'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/supplier/performance/:adminID/:from/:to/:supplier/:reqire_quantity/:type?branch=..........
// @description Get supplier wise performance report excel
// @access Private
router.get('/supplier/performance/:adminID/:from/:to/:supplier/:reqire_quantity/:type', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
        check('supplier', 'Supplier id is required').not().isEmpty()
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Supplier wise performance from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Suppler wise Performance From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('Supplier_wise_performance.xlsx'); // This is sails.js specific (in general you need to set headers)
