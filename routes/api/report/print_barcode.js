...
router.post('/print/barcode', [
    ...
    try {
        ...
        product_data.map(async (product, index) => {
            bwipjs.toBuffer({
                ...
                if (err) {
                    ...
                    }).select('_id name').populate(['availableSize']).populate('unitType', ['shortform']).populate('subcategory', ['name'])
                    ...
                    if (product_data.length == (index + 1)) {
                        ...
                        var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', 'reports', 'barcode', 'print.html'), 'utf8');
                        ...
                        var document = {
                            ...
                            path: "./public/reports/barcode_print.pdf"
                        };
                            ...
                            pdf.create(document, options)
                                .then(data => {
                                    ...
                                    return res.status(200).json({
                                        auth: true,
                                        fileLink: '/reports/barcode_print.pdf'
                                    })
                                })
                                .catch(error => {
                                    console.error(error)
