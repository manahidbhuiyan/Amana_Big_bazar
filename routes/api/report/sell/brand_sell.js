...
// @route GET /api/report/brand/sell/list?branch=..........
// @description Get brand wise sell report pdf
// @access Private
router.post('/brand/sell/list', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "brand_sell.js"));
        ...
        let reportFullDataset = {
            ...
            totalEarnAmount: totalBrandEarnAmount.toFixed(2),
            totalCostAmount: totalBrandCostAmount.toFixed(2),
            totalProfitAmount: totalProfitAmount.toFixed(2),
            specialDiscount: totalDiscountAmount.toFixed(2),
            netSellAmount: (totalBrandEarnAmount - totalDiscountAmount).toFixed(2),
            netProfitLoss: ((totalBrandEarnAmount - totalDiscountAmount) - totalBrandCostAmount).toFixed(2),
            netProfitLossPercentage: ((Math.abs((totalBrandEarnAmount - totalDiscountAmount) - totalBrandCostAmount) / totalBrandEarnAmount) * 100).toFixed(2),
        }

        var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'brand_wise_sell.html'), 'utf8');
        ...
        var options = {
            ...
            header: {
                ...
                contents: {
                    ...
                    class="table-border">
                    <tr>
                        <th style="font-size: 7px; text-align: left; width: 15%;">Brand Code</th>
                        <th style="font-size: 7px; text-align: left; width: 30%;">Brand Name</th>
        ...
        var document = {
            ...
            path: "./public/reports/brand_wise_sell_report.pdf"
        };
        ...
        pdf.create(document, options)
            .then(data => {
                ...
                res.status(200).json({
                    auth: true,
                    fileLink: '/reports/brand_wise_sell_report.pdf' 
                })
            })
            .catch(error => {
                console.error(error)
...
// @route GET /api/report/brand/sell/:adminID/:from/:to
// @description Get brand wise sell excel
// @access Private
router.get('/brand/sell/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response)=>{
            ...
            let grandTotal = {
                totalBrandEarnAmount: totalBrandEarnAmount,
                totalBrandCostAmount: totalBrandCostAmount,
                totalProfitAmount: totalProfitAmount,
                totalGp: totalGp,
                totalSoldQuantity: totalSoldQuantity,
                specialDiscount: totalDiscountAmount,
                netSellAmount: (totalBrandEarnAmount - totalDiscountAmount),
                netProfitLoss: ((totalBrandEarnAmount - totalDiscountAmount) - totalBrandCostAmount),
                netProfitLossPercentage: ((Math.abs((totalBrandEarnAmount - totalDiscountAmount) - totalBrandCostAmount) / totalBrandEarnAmount) * 100),
            }
            ...
            const specification = {
                serial: { // <- the key should match the actual data key
                    displayName: 'Brand Code', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 200, // <- width in pixels
                    cellStyle: styles.text_left
                },
                name: { // <- the key should match the actual data key
                    displayName: 'Brand Name', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
            ...
            const heading = [
                [{ value: 'Net Profit/Loss By Brand from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
                ...
                [{ value: 'Total Cost Amount: ' + grandTotal.totalBrandCostAmount, style: styles.shop_info }],
                ...
                ['Grand Total:', ' ',
                    ...
                    grandTotal.totalBrandEarnAmount,
                    grandTotal.totalBrandCostAmount,
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Net Profit/Loss By Brand From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('brand_wise_sell.xlsx'); // This is sails.js specific (in general you need to set headers)
