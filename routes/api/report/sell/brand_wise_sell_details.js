...
// @route GET api/report/brand/sell/details?branch=..........
// @description Get single brand wise sell report details pdf
// @access Private
router.post('/brand/sell/details', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "brand_wise_sell_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                ...
                supplier_name: brands[0].name,
                supplier_serialNo: brands[0].serialNo,
                data: sellList,
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'sell', 'brand_wise_sell_details.html'), 'utf8');
                ...
                header: {
                    ...
                    contents: {
                        ...
                        <tr>
                            ...
                            <th style="font-size: 7px; text-align: left; width: 29%;">Product</th>
            ...
            var document = {
                ...
                path: "./public/reports/brand_wise_sell_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/brand_wise_sell_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/brand/sell/:adminID/:from/:to/:brand?branch=..........
// @description Get single brand wise sell report details excel
// @access Private
router.get('/brand/sell/:adminID/:from/:to/:brand', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Brand wise Sell Details from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
                ...
                [''],
                [{ value: 'Brand Code: ' + reportFullDataset.supplier_serialNo, style: styles.shop_info }],
                [{ value: 'Brand Name: ' + reportFullDataset.supplier_name, style: styles.shop_info }],
                [''],
                ['Grand Total:', ' ', ' ', ' ',
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Brand wise Sell Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('brand_wise_sell_details.xlsx'); // This is sails.js specific (in general you need to set headers)
