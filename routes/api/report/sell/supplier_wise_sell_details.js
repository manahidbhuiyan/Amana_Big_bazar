...
// @route GET api/report/supplier/sell/details?branch=..........
// @description Get sinlge supplier wise sell report pdf 
// @access Private
router.post('/supplier/sell/details', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "supplier_wise_sell_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                ...
                supplier_serialNo: suppliers[0].serialNo,
                data: sellList,
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'sell', 'supplier_wise_sell_details.html'), 'utf8');
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 7px; text-align: left; width: 14%; padding-left: 3px">Bar Code</th>
                            <th style="font-size: 7px; text-align: left; width: 38%;">Product</th>
            ...
            var document = {
                ...
                path: "./public/reports/supplier_wise_sell_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/supplier_wise_sell_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/supplier/sell/:adminID/:from/:to/:supplier?branch=..........
// @description Get sinlge supplier wise sell report excel 
// @access Private
router.get('/supplier/sell/:adminID/:from/:to/:supplier', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Supplier wise Sell Details from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
                ...
                [''],
                [{ value: 'Supplier Code: ' + reportFullDataset.supplier_serialNo, style: styles.shop_info }],
                [{ value: 'Supplier Name: ' + reportFullDataset.supplier_name, style: styles.shop_info }],
                [''],
                ['Grand Total:', ' ', ' ', ' ',
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Supplier wise Sell Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('supplier_wise_sell_details.xlsx'); // This is sails.js specific (in general you need to set headers)
