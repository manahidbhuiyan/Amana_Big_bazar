...
// @route GET /api/report/brand/stock/summery/report?branch=..........
// @description Get brand wise stock report summery pdf
// @access Private
router.post('/brand/stock/summery/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "brand_stock_summery.js"));
        ...
        child.on('message', async (response) => {
            ...
            let branch = await Branch.findById(req.query.branch).select('_id serialNo name address phone');
                ...
                let reportFullDataset = {
                    title: "summery stock report by brand",
                    headers: ["Brand Code", "Brand Name"],
                ...
                var options = {
                    ...
                    header: {
                        ...
                        contents: {
                            ...
                            class="table-border">
                            <tr>
                                <th style="font-size: 8px; text-align: left; width: 15% ;padding-left: 3px">Brand Code</th>
                                <th style="font-size: 8px; text-align: left; width: 35%">Brand Name</th>
                                <th style="font-size: 8px; text-align: right;">Quantity</th>
                                <th style="font-size: 8px; text-align: right;">Cost Amt</th>
                                <th style="font-size: 8px; text-align: right;">Sales Amount</th>
                ...
                var document = {
                    ...
                    path: "./public/reports/brand_stock_report_summery.pdf"
                };
                ...
                pdf.create(document, options)
                    .then(data => {
                        ...
                        return res.status(200).json({
                            auth: true,
                            fileLink: '/reports/brand_stock_report_summery.pdf' 
                        })
                    })
                    .catch(error => {
                        console.error(error)
...
// @route GET /api/report/brand/stock/summary/:adminID/:brand/:zero_stock?branch=..........
// @description Get brand wise stock report summary excel
// @access Private
router.get('/brand/stock/summary/:adminID/:brand', async (req, res) => {
    try {
        const error = validationResult(req)
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Stock Summary Report By Brand At Date: ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Stock Summary Report By Brand At Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('brand_wise_stock_summary.xlsx'); // This is sails.js specific (in general you need to set headers)
