...
// @route GET /api/report/category/stock/report?branch=..........
// @description Get category wise summery stock report pdf
// @access Private
router.post('/category/stock/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "category_stock_summery.js"));
        const msg = {
        ...
        child.on('message', async (response) => {
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 8px; text-align: left; width: 15% ;padding-left: 3px">Category Code</th>
                            <th style="font-size: 8px; text-align: left; width: 42%">Category Name</th>
            ...
            var document = {
                ...
                path: "./public/reports/category_stock_report_summery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/category_stock_report_summery.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/category/stock/summary/:adminID/:category?branch=..........
// @description Get category wise summary stock report excel
// @access Private
router.get('/category/stock/summary/:adminID/:category', async (req, res) => {
    try {
        const error = validationResult(req)
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Stock Summary Report By Category At Date: ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Stock Summary Report By Category At Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('category_wise_stock_summary.xlsx'); // This is sails.js specific (in general you need to set headers)
