...
// @route GET api/report/category/under_stock/report?branch=..........
// @description Get category wise under stock report pdf
// @access Private
router.post('/category/under_stock/report', [
    adminAuth,
    [
        check('category', 'Supplier is required').not().isEmpty()
    ]
], async (req, res) => {
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "category_under_stock.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "Under stock report by Category",
            ...
            var document = {
                ...
                path: "./public/reports/category_under_stock_report.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/category_under_stock_report.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/category/under_stock/report/:adminID/:category?branch=..........
// @description Get category wise under stock report excel
// @access Private
router.get('/category/under_stock/report/:adminID/:category', [
    [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Category wise Under Stock Report On Date : ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Category wise Under Stock Report On Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('category_wise_under_stock.xlsx'); // This is sails.js specific (in general you need to set headers)
