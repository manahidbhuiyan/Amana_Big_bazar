...
// @route GET api/report//supplier/over_stock/report?branch=..........
// @description Get supplier wise over stock report pdf
// @access Private
router.post('/supplier/over_stock/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "supplier_over_stock.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "Over stock report by Supplier",
            ...
            var document = {
                ...
                path: "./public/reports/supplier_over_stock_report.pdf"
            };
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/supplier_over_stock_report.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report//supplier/over_stock/report/:adminID/:from/:to/:supplier?branch=..........
// @description Get supplier wise over stock report excel
// @access Private
router.get('/supplier/over_stock/report/:adminID/:from/:to/:supplier', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Supplier wise over stock from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Suppler wise over stock From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('supplier_wise_over_stock.xlsx'); // This is sails.js specific (in general you need to set headers)
