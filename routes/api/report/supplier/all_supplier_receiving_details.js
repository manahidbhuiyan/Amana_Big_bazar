...
// @route GET api/report/all/supplier-receiving/details?branch=..........
// @description Get all supplier receiving details pdf
// @access Private
router.post('/all/supplier-receiving/details', [
    ...
    try {
        ...
        let { from, to, receivingPlace, branch } = req.body
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'summery', 'all_supplier_receiving_details.html'), 'utf8');
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/all_supplier_receiving_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/all_supplier_receiving_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/all/supplier-receiving/details/:adminID/:from/:to?branch=..........
// @description Get all supplier receiving details excel
// @access Private
router.get('/all/supplier-receiving/details/:adminID/:from/:to/:branch/:receivingPlace', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'All Supplier Receiving Details('  + branchNum + ' From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'All Supplier Receiving Details(' + branchNum + 'From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
