...
// @route GET api/report/all/supplier-receiving/summery/?branch=..........
// @description Get all supplier receiving summery pdf
// @access Private
router.post('/all/supplier-receiving/summery', [
    auth,
    [
    ...
    try {
        ...
        let { from, to, branch, receivingPlace } = req.body
        ...
        const child = fork(path.join(__dirname, "subprocess", "all_supplier_receiving_summery.js"));
        ...
        child.on('message', async (response) => {
            ...
            let allDataInfo = {
                ...
                title : branch == 'all' ? "All Supplier Receiving Summary(all branch)" : "All Supplier Receiving Summary(single branch)",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'summery', 'all_supplier_receiving_summery.html'), 'utf8');
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                            <tr>
                                <th style="font-size: 8px; text-align: left; width: 15% ;padding-left: 3px">Supplier</th>
                                <th style="font-size: 8px; text-align: left; width: 13%">Receiving No</th>
                                <th style="font-size: 8px; text-align: right;">Receive Qty</th>
                                <th style="font-size: 8px; text-align: center;">Total Cost</th>
                                <th style="font-size: 8px; text-align: center;">Total Sell</th>
                                <th style="font-size: 8px; text-align: center;">GP(%)</th>
                            </tr>
                        </table>`
                    }
                },
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/all_supplier_receiving_summery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/all_supplier_receiving_summery.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/all/supplier-receiving/summery/:adminID/:from/:to?branch=..........
// @description Get all supplier receiving summery excel
// @access Private
router.get('/all/supplier-receiving/summery/:adminID/:from/:to/:branch/:receivingPlace', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'All Supplier Receiving Summery(' + branchNum + ' From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'All Supplier Receiving Summery(' + branchNum + ' From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('all_supplier_receiving_summery.xlsx'); // This is sails.js specific (in general you need to set headers)
