...
// @route GET api/report/all/supplier-return/details?branch=..........
// @description Get all supplier return report details pdf 
// @access Private
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            let allDataInfo = {
                ...
                title : branch == 'all' ? "All Supplier Receiving details(all branch)" : "All Supplier Receiving details(single branch)",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'summery', 'all_supplier_return_details.html'), 'utf8');
                    ...
                    contents: {
                        ...
                        class="table-border">
                            <tr>
                                ...
                                <th style="font-size: 9px; text-align: center;">Total Sell</th>
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/all_supplier_return_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/all_supplier_return_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/all/supplier-return/details/:adminID/:from/:to/:branch=..........
// @description Get all supplier return report details excel 
// @access Private
router.get('/all/supplier-return/details/:adminID/:from/:to/:branch', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'All Supplier Return Details(' + branchNum + ' From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'All Supplier Return Details(' + branchNum + ' From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('all_supplier_return_details.xlsx'); // This is sails.js specific (in general you need to set headers)
