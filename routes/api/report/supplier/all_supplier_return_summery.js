...
// @route GET api/report//all/supplier-return/summery?branch=..........
// @description Get all supplier return report summery pdf
// @access Private
router.post('/all/supplier-return/summery', [
    auth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "all_supplier_return_summery.js"));
        ...
        child.on('message', async (response) => {
            ...
            let allDataInfo = {
                ...
                title : branch == 'all' ? "All Supplier Return Summary(all branch)" : "All Supplier Return Summary(single branch)",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'summery', 'all_supplier_return_summery.html'), 'utf8');
                    ...
                    contents: {
                        ...
                        class="table-border">
                            <tr>
                                ...
                                <th style="font-size: 10px; text-align: left; width: 25%">Return No</th>
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/all_supplier_return_summery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/all_supplier_return_summery.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report//all/supplier-return/summery/:adminID/:from/:to?branch=..........
// @description Get all supplier return report summery excel
// @access Private
router.get('/all/supplier-return/summery/:adminID/:from/:to/:branch', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
        ...
        child.on('message', async (response) => {
            ...
            const specification = {
                ...
                productToatalSell: { // <- the key should match the actual data key
                    displayName: 'Return Total Sell', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 100 // <- width in pixels
                },
                gp: { // <- the key should match the actual data key
                    displayName: 'Return Gp(%)', // <- Here you specify the column header
                    headerStyle: styles.headerDark, // <- Header style
                    width: 100 // <- width in pixels
                }
            }
            ...
            const heading = [
                [{ value: 'All Supplier Return Summery(' + branchNum + ' From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'All Supplier Return Summery(' + branchNum + ' From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('all_supplier_return_summery.xlsx'); // This is sails.js specific (in general you need to set headers)
