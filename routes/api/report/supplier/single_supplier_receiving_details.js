...
// @route GET api/report/single/supplier-receiving/details?branch=..........
// @description Get single supplier receiving report details pdf 
// @access Private
router.post('/single/supplier-receiving/details', [
    auth,
    [
    ...
    try {
        ...
        to.setMinutes (59);
        to.setSeconds (59);
        const child = fork(path.join(__dirname, "subprocess", "single_supplier_receiving_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let allDataInfo = {
                ...
                title : branch == 'all' ? "Single Supplier Receiving details(all branch)" : "Single Supplier Receiving details(single branch)",
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'summery', 'single_supplier_receiving_details.html'), 'utf8');
                    ...
                    contents: {
                        ...
                        class="table-border">
                            <tr>
                                ...
                                <th style="font-size: 9px; text-align: right; width: 15%">Unit Sell</th>
                                <th style="font-size: 9px; text-align: right;">Receive Qty</th>
                                <th style="font-size: 9px; text-align: right;">Total Cost</th>
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/single_supplier_receiving_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/single_supplier_receiving_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/single/supplier-receiving/details/:adminID/:from/:to/:supplier?branch=..........
// @description Get single supplier receiving report details excel 
// @access Private
router.get('/single/supplier-receiving/details/:adminID/:from/:to/:supplier/:branch/:receivingPlace', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
        check('supplier', 'Supplier id is required').not().isEmpty()
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Single Supplier Receiving Details('  + branchNum + ' From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Single Supplier Receiving Details('  + branchNum + ' From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('single_supplier_receiving_details.xlsx'); // This is sails.js specific (in general you need to set headers)
