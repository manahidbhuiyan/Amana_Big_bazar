...
// @route GET api/report/single/supplier-return/summery?branch=..........
// @description Get single supplier return report summery pdf
// @access Private
router.post('/single/supplier-return/summery', [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'summery', 'single_supplier_return_summery.html'), 'utf8');
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/single_supplier_return_summery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/single_supplier_return_summery.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/single/supplier-return/summery/:adminID/:from/:to/:supplier?branch=..........
// @description Get single supplier return report summery excel
// @access Private
router.get('/single/supplier-return/summery/:adminID/:from/:to/:supplier/:branch', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Single Supplier Return Summery(' + branchNum + ' From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Single Supplier Return Summery(' + branchNum + ' From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('single_supplier_return_summery.xlsx'); // This is sails.js specific (in general you need to set headers)
