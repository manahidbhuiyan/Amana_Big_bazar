...
process.on('message', async (msg) => {
    ...
    let branchArray = await branches.map(async (branchInfo) => {
        let finalArray = await supplierOfBranch.map(async (supplierInfo, index) => {
            ...
            let childArray = await supplierRecevingData.map(async (receivingInfo, index) => {
                ...
                singleReceivingInfo.push(receivingInfo.fromWarehouse ? 'Yes' : 'No')
            ...
            if (supplierInfoDataList.length > 0) {
                allSupplierInfo.push({
                    ...
                    fromWarehouse : receivingPlace == 'both' ? 'both' : (receivingPlace == 'fromWarehouse' ? 'Warehouse' : 'Supplier')
                })
            }
        })
            ...
            supplierInfo.allSupplierData.map((info) => {
                ...
                info.data.map((productsInfo, index) => {
                    ...
                    if (index == 0) {
                        ...
                        dataObj.fromWarehouse = productsInfo[6]
                    }
