...
// @route GET api/report/all/supplier-requisition/details?branch=..........
// @description Get all supplier requisition details pdf
// @access Private
router.post('/all/supplier-requisition/details', [
    auth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "all_supplier_requisition_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'supplier_requisition', 'all_supplier_requisition_details.html'), 'utf8');
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/all_supplier_requisition_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/all_supplier_requisition_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/all/supplier-requisition/details/:adminID/:from/:to?branch=..........
// @description Get all supplier requisition details excel
// @access Private
router.get('/all/supplier-requisition/details/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'All Supplier requisition Details From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'All Supplier requisition Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('all_supplier_requisition_details.xlsx'); // This is sails.js specific (in general you need to set headers)
