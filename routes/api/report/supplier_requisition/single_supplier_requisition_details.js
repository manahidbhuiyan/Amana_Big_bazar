...
// @route GET api/report/single/supplier-requisition/details?branch=..........
// @description Get single supplier requisition report details pdf 
// @access Private
router.post('/single/supplier-requisition/details', [
    auth,
    [
    ...
    try {
        ...
        to.setMinutes (59);
        to.setSeconds (59);
        const child = fork(path.join(__dirname, "subprocess", "single_supplier_requisition_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', 'reports', 'supplier_requisition', 'single_supplier_requisition_details.html'), 'utf8');
                ...
                header: {
                    ...
                    contents: {
                        ...
                        <tr>
                            ...
                            <th style="font-size: 8px; text-align: right;">Stock Qty</th>
                            <th style="font-size: 8px; text-align: right; width: 14%">Req Qty</th>
                            <th style="font-size: 8px; text-align: right;">Unit Cost</th>
                            <th style="font-size: 8px; text-align: right; width: 12%">Unit Sell</th>
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/single_supplier_requisition_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/single_supplier_requisition_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/single/supplier-requisition/details/:adminID/:from/:to/:supplier?branch=..........
// @description Get single supplier requisition report details excel 
// @access Private
router.get('/single/supplier-requisition/details/:adminID/:from/:to/:supplier', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
        check('supplier', 'Supplier id is required').not().isEmpty()
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Single Supplier Requisition Details From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Single Supplier Requisition Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('single_supplier_requisition_details.xlsx'); // This is sails.js specific (in general you need to set headers)
