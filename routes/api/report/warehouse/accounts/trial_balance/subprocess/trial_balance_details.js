...
process.on('message', async (msg) => {
    ...
    let subcategoriesArray = subcategories.map( async (subcategoryInfo) => { 
        ...
        finalData.push({
            ...
            totalDebit : 0,
            totalCredit : 0,
            closingBalance : (subcategoryInfo.warehouse_opening_balance + closingBalance) 
        })
        trialBalanceTotalClosingBalance += (subcategoryInfo.warehouse_opening_balance + closingBalance) 
    })
    await Promise.all(subcategoriesArray)
    ...
    let suppliersArray = suppliers.map( async (supplierInfo) => {    
        ...
        if(journalsForSupplier.length > 0){
            ...
            finalData.push({
                ...
                totalDebit : 0,
                totalCredit : 0,
                closingBalance : (supplierInfo.warehouseOpeningBalance ? (supplierInfo.warehouseOpeningBalance + supplierAmount) : supplierAmount) 
            })
            trialBalanceTotalClosingBalance += (supplierInfo.warehouseOpeningBalance ? (supplierInfo.warehouseOpeningBalance + supplierAmount) : supplierAmount)
        }
    })
            ...
            if(supplierIndex != -1){
                ...
                if(journalInfo.isCredit){
                    finalData.push({
                        ...
                        totalDebit : 0,
                        totalCredit : journalInfo.amount,
                        closingBalance : (journalInfo.supplier.warehouseOpeningBalance + journalInfo.amount )
                    })
                    trialBalanceTotalCredit += journalInfo.amount
                    trialBalanceTotalClosingBalance += (journalInfo.supplier.warehouseOpeningBalance + journalInfo.amount)
                }else{
                    finalData.push({
                        particular : journalInfo.supplier.name,
                        totalDebit : journalInfo.amount,
