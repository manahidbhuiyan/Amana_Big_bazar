    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_trial_balance_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_trial_balance_details.pdf',
                        filename: 'warehouse_trial_balance_details'
                    })
                })
                .catch(error => {
                    console.error(error)
...
router.get('/accounts/trial_balance/details/:adminID/:from/:to/:group/:subgroup/:category', async (req, res) => {
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Trial Balance Details report date wise from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Warehouse Trial Balance Details report date wise from ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_trial_balance_details_date_wise.xlsx'); // This is sails.js specific (in general you need to set headers)
