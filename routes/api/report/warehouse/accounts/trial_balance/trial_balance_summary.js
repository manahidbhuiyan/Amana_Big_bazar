...
// @route GET /accounts/trial_balance/pdf?from=?&to=?&branch=?&group=?&subgroup=?&category=?&subcategory=?
// @description Get warehouse trial_balance summary report
// @access Private
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_trial_balance_summary.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_trial_balance_summary.pdf',
                        filename: 'warehouse_trial_balance_summary'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// description Get warehouse trial balance summary Excel report
// @access Private
router.get('/accounts/trial_balance/summary/:adminID/:from/:to', async (req, res) => {
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Trail Balance Summary Report At Date: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Warehouse Trail Balance Summary Report At Date: ' + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_trial_balance_summary_date_wise.xlsx'); // This is sails.js specific (in general you need to set headers)
