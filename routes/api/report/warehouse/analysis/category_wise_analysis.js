...
// @route GET api/report/single/category/analysis
...
router.post('/single/category/analysis', [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..','..', 'reports', 'analysis', 'category_wise_analysis.html'), 'utf8');
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/category_wise_analysis.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/category_wise_analysis.pdf',
                        filename: 'category_wise_analysis'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/single/category/analysis/:adminID/:from/:to/:category?branch=..........
// @description Get category wise analysis report excel
// @access Private
router.get('/single/category/analysis/:adminID/:from/:to/:category', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
        check('category', 'Category id is required').not().isEmpty()
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Category wise analysis from : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Category wise analysis From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('category_wise_analysis.xlsx'); // This is sails.js specific (in general you need to set headers)
