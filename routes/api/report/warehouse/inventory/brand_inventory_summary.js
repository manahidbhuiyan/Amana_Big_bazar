...
// @route GET api/report/brand/inventory/summery/report
// @description Get brand inventory report summery pdf
// @access Private
router.post('/brand/inventory/summary/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "brand_inventory_summary.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "warehouse inventory report summary by brand",
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 10px; text-align: left; width: 12% ;padding-left: 3px">Brand Code</th>
                            <th style="font-size: 10px; text-align: left; width: 30%">Brand Name</th>
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_brand_inventory_report_summary.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_brand_inventory_report_summary.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/brand/inventory/summery/report/:adminID/:brand
// @description Get brand inventory report summery excel
// @access Private
router.get('/brand/inventory/summary/report/:adminID/:brand', [
    [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Brand wise warehouse Inventory Stock Summary On Date : ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Brand wise warehouse Inventory Stock Summary On Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('brand_wise_warehouse_inventory_stock_summary.xlsx'); // This is sails.js specific (in general you need to set headers)
