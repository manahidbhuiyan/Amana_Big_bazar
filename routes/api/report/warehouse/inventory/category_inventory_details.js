...
// @route GET api/report/category/inventory/details/report
// @description Get category inventory report details pdf
// @access Private
router.post('/category/inventory/details/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "category_inventory_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "detail warehouse inventory report by category",
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_category_inventory_report_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_category_inventory_report_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/category/inventory/details/report/:adminID/:category
// @description Get category inventory report details excel
// @access Private
router.get('/category/inventory/details/report/:adminID/:category', [
    [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Category wise warehouse Inventory Stock Details On Date : ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Category wise warehouse Inventory Stock Details On Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('category_wise_warehouse_inventory_stock_details.xlsx'); // This is sails.js specific (in general you need to set headers)
