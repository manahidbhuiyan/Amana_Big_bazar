...
// @route GET api/report/category/inventory/summary/report
// @description Get category inventory report summary pdf
// @access Private
router.post('/category/inventory/summary/report', [
    adminAuth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "category_inventory_summary.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "warehouse inventory report summary by category",
            ...
            var options = {
                ...
                header: {
                    ...
                    contents: {
                        ...
                        class="table-border">
                        <tr>
                            <th style="font-size: 10px; text-align: left; width: 12% ;padding-left: 3px">Category Code</th>
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_category_inventory_report_summary.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_category_inventory_report_summary.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/category/inventory/summary/report/:adminID/:category
// @description Get category inventory report summary excel
// @access Private
router.get('/category/inventory/summary/report/:adminID/:category', [
    [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Category wise warehouse Inventory Stock Summary On Date : ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Category wise warehouse Inventory Stock Summary On Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('category_wise_warehouse_inventory_stock_summary.xlsx'); // This is sails.js specific (in general you need to set headers)
