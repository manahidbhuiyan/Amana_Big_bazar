...
process.on('message', async (msg) => {
    ...
    let allCategoryData = []
    let supplierParentArray = suppliers.map(async (supplier, index) => {
        let productFindCondition = {
            supplier: supplier._id
        }
        let supplierWiseProductList = await Product.find(productFindCondition)
        ...
        let categoryWiseProductChildOne = supplierWiseProductList.map(async (productInfo, index) => {
            let supplierWiseInventoryProduct = await Inventory.find({ product: productInfo._id })
            if (supplierWiseInventoryProduct.length !== 0) {
                let currentStock = productInfo.quantity
                let inventoryStock = supplierWiseInventoryProduct[0].stock_quantity
