...
// @route GET api/report/supplier/inventory/details/report
// @description Get supplier inventory report details pdf
// @access Private
router.post('/supplier/inventory/details/report', [
    adminAuth,
    [
    ...
    try {
        ...
        if (supplier != "all") {
            ...
            condition.warehouseSupplier = true
            condition.activeSupplier = true
        }

        const child = fork(path.join(__dirname, "subprocess", "supplier_inventory_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "detail warehouse inventory report by supplier",
                ...
                header: {
                    ...
                    contents: {
                        ...
                        <tr>
                            ...
                            <th style="font-size: 10px; text-align: center;">C.Price</th>
                            <th style="font-size: 10px; text-align: center;">S.Price</th>
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_supplier_inventory_report_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_supplier_inventory_report_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/supplier/inventory/details/report/:adminID/:supplier
// @description Get supplier inventory report details excel
// @access Private
router.get('/supplier/inventory/details/report/:adminID/:supplier', [
    [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Supplier wise warehouse Inventory Stock Details On Date : ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Supplier wise warehouse Inventory Stock Details On Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('supplier_wise_warehouse_inventory_stock_details.xlsx'); // This is sails.js specific (in general you need to set headers)
