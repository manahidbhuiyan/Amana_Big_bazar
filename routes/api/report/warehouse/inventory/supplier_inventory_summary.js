...
// @route GET api/report/supplier/inventory/summary/report
// @description Get supplier inventory report summary pdf
// @access Private
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "warehouse inventory report summary by supplier",
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_supplier_inventory_report_summary.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_supplier_inventory_report_summary.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/supplier/inventory/summary/report/:adminID/:supplier
// @description Get supplier inventory report summary excel
// @access Private
router.get('/supplier/inventory/summary/report/:adminID/:supplier', [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Supplier wise warehouse Inventory Stock Summary On Date : ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Supplier wise warehouse Inventory Stock Summary On Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('supplier_wise_warehouse_inventory_stock_summary.xlsx'); // This is sails.js specific (in general you need to set headers)
