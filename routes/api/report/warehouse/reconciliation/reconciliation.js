...
// @route GET api/report/warehouse/product/reconciliation
// @description Ge warehouse product reconciliation report pdf
// @access Private
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_reconciliation_report.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_reconciliation_report.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/warehouse/product/reconciliation/:adminID/:from/:to
// @description Get warehouse product reconciliation report excel
// @access Private
router.get('/product/reconciliation/:adminID/:from/:to', async (req, res) => {
    try {
        ...
        const adminRoles = await getAdminRoleChecking(adminInformation.id, 'warehouse report')

        if (!adminRoles) {
            return res.status(400).send({
                errors: [
                    {
                        msg: 'Account is not authorized to warehouse report'
                    }
                ]
            })
        }
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Product Reconciliation report from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'warehouse product reconciliation report from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_product_reconciliation_report.xlsx'); // This is sails.js specific (in general you need to set headers)
...
// @route GET api/report/warehouse/inventory/product/reconciliation
// @description Ge warehouse product reconciliation report pdf
// @access Private
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var document = {
                html: html,
                data: reportFullDataset,
                path: "./public/reports/warehouse_inventory_reconciliation_report.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_inventory_reconciliation_report.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET api/report/warehouse/inventory/product/reconciliation/:adminID/:from/:to
// @description Get warehouse product reconciliation report excel
// @access Private
router.get('/inventory/product/reconciliation/:adminID/:from/:to', async (req, res) => {
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Inventory Product Reconciliation report from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
                [''], // <-- It can be only values
                [{ value: 'Shop Name: ' + reportFullDataset.branch_name, style: styles.shop_info }],
                [{ value: 'Shop Address: ' + reportFullDataset.branch_address, style: styles.shop_info }],
                [{ value: '', style: styles.cell_border }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'warehouse inventory product reconciliation report from: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
                        specification: specification, // <- Report specification
                        data: reportFullDataset.data // <-- Report data
            ...
            // You can then return this straight
            res.attachment('warehouse_inventory_product_reconciliation_report.xlsx'); // This is sails.js specific (in general you need to set headers)
            return res.send(report);
        })
        child.on('close', (code) => {
            child.kill()
