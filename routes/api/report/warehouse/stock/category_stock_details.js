...
// @route GET api/report/catgory/stock/details/report
// @description Get all catgories stock report details pdf
// @access Private
router.post('/category/stock/details/report', [
    adminAuth,
    [
    ...
    try {
        ...
        let { category, zero_stock } = req.body
        ...
        const child = fork(path.join(__dirname, "subprocess", "category_stock_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            let reportFullDataset = {
                title: "detail stock report by category",
                headers: ["BraCode", "Product Name"],
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_category_stock_report_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_category_stock_report_details.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
// @route GET /api/report/category/stock/details/:adminID/:category/:zero_stock?branch=..........
// @description Get category wise details stock report excel
// @access Private
router.get('/category/stock/details/:adminID/:category/:zero_stock', async (req, res) => {
    try {
        const error = validationResult(req)
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Stock Details Report By Category At Date: ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Warehouse Stock Details Report By Category At Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_category_wise_stock_details.xlsx'); // This is sails.js specific (in general you need to set headers)
