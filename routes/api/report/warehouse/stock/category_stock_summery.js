...
// @route GET api/report/category/stock/report
// @description Get all categories stock report summery pdf
// @access Private
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var document = {
                ...
                path: "./public/reports/warehouse_category_stock_report_summery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_category_stock_report_summery.pdf'
                    })
                })
                .catch(error => {
                    console.error(error)
...
router.get('/category/stock/summary/:adminID/:category', async (req, res) => {
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Stock Summary Report By Category At Date: ' + reportFullDataset.currentDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Warehouse Stock Summary Report By Category At Date: ' + reportFullDataset.currentDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_category_wise_stock_summary.xlsx'); // This is sails.js specific (in general you need to set headers)
