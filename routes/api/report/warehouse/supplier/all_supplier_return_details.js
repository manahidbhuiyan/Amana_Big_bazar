...
// @route GET api/report/warehouse/all/supplier-return/details
// @description Get warehouse all suppliers return details pdf
// @access Private
router.post('/all/supplier-return/details', [
    auth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "all_supplier_return_details.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', '..', 'reports', 'warehouse', 'summery', 'all_supplier_return_details.html'), 'utf8');
                    ...
                    contents: {
                        ...
                        class="table-border">
                            <tr>
                                ...
                                <th style="font-size: 8px; text-align: right;">Unit Sell</th>
                                <th style="font-size: 8px; text-align: center;">Return Qty</th>
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/warehouse_all_supplier_return_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_all_supplier_return_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/warehouse/all/supplier-return/details/:adminID/:from/:to?branch=..........
// @description Get warehouse all supplier return report details excel 
// @access Private
router.get('/all/supplier-return/details/:adminID/:from/:to', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse All Supplier Return Details From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Warehouse All Supplier Return Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_all_supplier_return_details.xlsx'); // This is sails.js specific (in general you need to set headers)
