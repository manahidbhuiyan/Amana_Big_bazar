...
// @route GET api/report/warehouse/single/supplier-receiving/details
// @description Get warehouse single supplier receiving report details pdf
// @access Private
router.post('/single/supplier-receiving/details', [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', '..', 'reports', 'warehouse', 'summery', 'single_supplier_receiving_details.html'), 'utf8');
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/warehouse_single_supplier_receiving_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_single_supplier_receiving_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/warehouse/single/supplier-receiving/details/:adminID/:from/:to/:supplier?branch=..........
// @description Get warehouse single supplier receiving report details excel 
// @access Private
router.get('/single/supplier-receiving/details/:adminID/:from/:to/:supplier', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
        check('supplier', 'Supplier id is required').not().isEmpty()
