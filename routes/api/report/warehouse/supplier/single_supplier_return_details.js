...
// @route GET api/report/warehouse/single/supplier-return/details
// @description Get warehouse single supplier return report details pdf
// @access Private
router.post('/single/supplier-return/details', [
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', '..', 'reports', 'warehouse', 'summery', 'single_supplier_return_details.html'), 'utf8');
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/warehouse_single_supplier_return_details.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_single_supplier_return_details.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/warehouse/single/supplier-return/details/:adminID/:from/:to/:supplier?branch=..........
// @description Get warehouse  single supplier return report details excel
// @access Private
router.get('/single/supplier-return/details/:adminID/:from/:to/:supplier', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
        check('supplier', 'Supplier id is required').not().isEmpty()
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Single Supplier Return Details From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Warehouse Single Supplier Return Details From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_single_supplier_return_details.xlsx'); // This is sails.js specific (in general you need to set headers)
