...
// @route GET api/report/warehouse/single/supplier-return/summery
// @description Get warehouse single supplier return report summery pdf
// @access Private
router.post('/single/supplier-return/summery', [
    auth,
    [
    ...
    try {
        ...
        const child = fork(path.join(__dirname, "subprocess", "single_supplier_return_summery.js"));
        ...
        child.on('message', async (response) => {
            ...
            var html = fs.readFileSync(path.join(__dirname, '..', '..', '..', '..', '..', 'reports', 'warehouse', 'summery', 'single_supplier_return_summery.html'), 'utf8');
                    ...
                    contents: {
                        ...
                        class="table-border">
                            <tr>
                                ...
                                <th style="font-size: 10px; text-align: left; width: 20%">Return No</th>
            ...
            var document = {
                html: html,
                data: allDataInfo,
                path: "./public/reports/warehouse_single_supplier_return_summery.pdf"
            };
            ...
            pdf.create(document, options)
                .then(data => {
                    ...
                    return res.status(200).json({
                        auth: true,
                        fileLink: '/reports/warehouse_single_supplier_return_summery.pdf'
                    })
                    //res.download(file);
                })
                .catch(error => {
...
// @route GET api/report/warehouse/single/supplier-return/summery/:adminID/:from/:to/:supplier?branch=..........
// @description Get warehouse single supplier return report summery excel
// @access Private
router.get('/single/supplier-return/summery/:adminID/:from/:to/:supplier', [
    [
        check('from', 'From date is required').not().isEmpty(),
        check('to', 'To date is required').not().isEmpty(),
    ]
    ...
    try {
        ...
        child.on('message', async (response) => {
            ...
            const heading = [
                [{ value: 'Warehouse Single Supplier Return Summery From : ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, style: styles.headerDark }],
            ...
            const report = excel.buildExport(
                [ // <- Notice that this is an array. Pass multiple sheets to create multi sheet report
                    {
                        name: 'Warehouse Single Supplier Return Summery From: ' + reportFullDataset.fromDate + ' To: ' + reportFullDataset.toDate, // <- Specify sheet name (optional)
                        heading: heading,
                        merges: merges,
            ...
            res.attachment('warehouse_single_supplier_return_summery.xlsx'); // This is sails.js specific (in general you need to set headers)
