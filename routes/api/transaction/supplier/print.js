...
router.post('/print', [
    ...
    try {
        ...
        let supplierRequisitionData = await RequisitionToSupplier.findById(download_id).populate('supplier', '_id name contact').populate('admin', 'name').populate('branch', 'name address phone serialNo')
        ...
        allDataInfo.branch_serial = supplierRequisitionData.branch.serialNo
        allDataInfo.branch_phone = supplierRequisitionData.branch.phone
        ...
        var document = {
            html: html,
            data: allDataInfo,
            path: "./public/reports/supplier_request.pdf"
        };
        ...
        pdf.create(document, options)
            .then(data => {
                ...
                return res.status(200).json({
                    auth: true,
                    fileLink: '/reports/supplier_request.pdf' 
                })
            })
            .catch(error => {
                console.error(error)
