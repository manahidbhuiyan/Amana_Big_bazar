...
router.post('/', [auth, 
    ...
    try {
        ...
        let { receivingID, supplier, purpose, remarks, products, totalAmount, totalQuantity, branch, toWarehouse } = req.body
        let customErrors = []

        let serialNo =  Number(String(branch.serialNo)+100000)
        
        const lastSupplierReturn = await ReturnToSupplier.findOne({branch:branch.id}).populate('supplier', ['name']).sort({create: -1})
        ...
        let checkProductStock = products.map(async productData =>{
            ...
            if(productInfo != null){
                if(productInfo.quantity < productData.quantity){
                    customErrors.push({
                        msg: 'Stock quantity(' + productInfo.quantity + ') is lower than Return qunatity(' + productData.quantity + ') for ' + productData.name
                    })
                }
            }
