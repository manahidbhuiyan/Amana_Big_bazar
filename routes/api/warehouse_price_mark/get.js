...
router.get('/:pageNo', auth, async (req, res) => {
    ...
    try {
        ...
        let priceMarkUpDownData = await priceMarkUpDown.find(condition).sort({create:'desc'}).limit(dataList).skip(offset)
    
        let totalProductNumber = await priceMarkUpDown.find(condition).countDocuments();
        let loadMore = (offset + priceMarkUpDownData.length) >= totalProductNumber ? false : true

        res.status(200).json({
            data: priceMarkUpDownData,
...
router.get('/dashboard/:pageNo', auth, async (req, res) => {
    ...
    try {
        ...
        let priceMarkUpDownData = await priceMarkUpDown.find(condition).populate('admin', '_id name').sort({create:'desc'}).limit(dataList).skip(offset)
