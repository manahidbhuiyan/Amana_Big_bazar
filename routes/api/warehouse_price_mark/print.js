    ...
    try {
        ...
        if (!adminRoles) {
            return res.status(400).send({
                errors: [
                    {
                        msg: 'Account is not authorized to warehouse product up-down'
                    }
                ]
            })
        }
        ...
        let priceMarkUpdown = await PriceMarkUpDown.findById(download_id).populate('admin', 'name')
        ...
        var document = {
            html: html,
            data: allDataInfo,
            path: "./public/reports/warehouse_price_mark_status.pdf"
        };
        ...
        pdf.create(document, options)
            .then(data => {
                ...
                return res.status(200).json({
                    auth: true,
                    fileLink: '/reports/warehouse_price_mark_status.pdf'
                })
            })
            .catch(error => {
                console.error(error)
