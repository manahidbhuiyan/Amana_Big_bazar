...
router.put('/', [auth,
    ...
    let receiveFromSupplier = await ReceivingFromSupplier.findOne({
    ...
    try {
        ...
        if(String(barcode).trim()!=String(product.barcode).trim()){
            ...
            let requisitionToSupplierList = await RequisitionToSupplier.find({
                "products.product.barcode": String(product.barcode).trim(),
        ...
        if (online_active) {
            ...
            await BranchProduct.updateMany({barcode: product.barcode}, {$set: {online_active: online_active}})
        }
    ...
    if (uploadedFileDetails.mimetype === 'image/png' || uploadedFileDetails.mimetype === 'image/jpeg') {
        ...
        if (filesize < 1) {
            try {
                ...
                if(req.body.imageAllBranch){
                    await BranchProduct.updateMany({barcode: product.barcode}, {$set: {images: product.images}})
                }
