...
/* GET login page. */
router.get('/view', urlAuth('inventory view'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'view',
      link: config.get('hostname') + '/dashboard/inventory/view'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Inventory Product View",
    pageTitle: "Inventory Product View"
  }

  res.render('pages/inventory/view', {
    scripts: scripts,
