...
/* GET POS Order Refund Page */
router.get('/refund/:id', urlAuth('pos order list refund'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [
      {
        text: 'refund',
        link: config.get('hostname') + '/dashboard/pos/order/refund/'+req.params.id
      }
    ]
  }
  ...
  res.render('pages/pos_order/refund', {
    scripts: scripts,
