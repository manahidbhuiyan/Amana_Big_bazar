...
/* GET login page. */
router.get('/reconciliation', urlAuth('product reconciliation'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'reconciliation',
      link: config.get('hostname') + '/dashboard/product/reconciliation'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Product Reconciliation",
    pageTitle: "Product Reconciliation"
  }

  res.render('pages/product/product_reconciliation', {
    scripts: scripts,
