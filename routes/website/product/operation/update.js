...
/* GET login page. */
router.get('/update/:id', urlAuth('product update'), function(req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [
      {
        text: 'update',
        link: config.get('hostname') + '/dashboard/product/update/'+req.params.id
      }
    ]
  }

  const data = {
    siteTitle: config.get('company').name + " - Product Update",
