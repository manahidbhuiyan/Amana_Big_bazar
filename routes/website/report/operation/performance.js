...
/* GET login page. */
router.get('/performance', urlAuth('report performance'), function(req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [
      {
        text: 'performance',
        link: config.get('hostname') + '/dashboard/report/performance'
      }
    ]
  }
  ...
  res.render('pages/report/performance', {
    scripts: scripts,
    styles: styles,
    breadcumb: breadcumb,
    data: data,
