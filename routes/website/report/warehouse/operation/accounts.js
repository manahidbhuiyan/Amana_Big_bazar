...
/* GET login page. */
router.get('/accounts', urlAuth('warehouse report accounts'), function(req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [
      {
        text: 'accounts',
        link: config.get('hostname') + '/dashboard/report/warehouse/accounts'
      }
    ]
  }
  ...
  const data = {
    ...
    pageTitle: "Warehouse Accounts Report",
    adminInfo: req.adminInfo,
  }

  res.render('pages/report/warehouse/accounts', {
    scripts: scripts,
    styles: styles,
    breadcumb: breadcumb,
    data: data,
