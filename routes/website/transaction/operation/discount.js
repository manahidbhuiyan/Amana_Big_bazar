...
/* GET Branch Transaction Discount */
router.get('/manage-discount', urlAuth('transaction manage discount'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'discount',
      link: config.get('hostname') + '/dashboard/transaction/manage-discount'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Transaction",
    pageTitle: "Manage Dicount",
    adminInfo: req.adminInfo,
  }

  res.render('pages/transaction/discount', {
    scripts: scripts,
