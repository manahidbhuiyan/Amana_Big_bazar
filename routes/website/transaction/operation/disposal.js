...
/* GET Branch Transaction Disposal */
router.get('/manage-disposal', urlAuth('transaction manage disposal'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'disposal',
      link: config.get('hostname') + '/dashboard/transaction/manage-disposal'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Transaction",
    pageTitle: "Manage Disposal",
    adminInfo: req.adminInfo,
  }

  res.render('pages/transaction/disposal', {
    scripts: scripts,
