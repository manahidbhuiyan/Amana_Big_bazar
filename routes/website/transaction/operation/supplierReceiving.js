...
/* GET Branch Transaction Receiving */
router.get('/supplier-receive', urlAuth('transaction supplier receive'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'receive',
      link: config.get('hostname') + '/dashboard/transaction/supplier-receive'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Transaction",
    pageTitle: "Receive From Supplier",
    adminInfo: req.adminInfo,
  }

  res.render('pages/transaction/supplierReceiving', {
    scripts: scripts,
