...
/* GET Branch Transaction Vat */
router.get('/manage-vat', urlAuth('transaction manage vat'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'vat',
      link: config.get('hostname') + '/dashboard/transaction/manage-vat'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Transaction",
    pageTitle: "Manage vat",
    adminInfo: req.adminInfo,
  }

  res.render('pages/transaction/vat', {
    scripts: scripts,
