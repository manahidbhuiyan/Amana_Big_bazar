...
/* GET login page. */
router.get('/reconciliation', urlAuth('warehouse inventory reconciliation'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'reconciliation',
      link: config.get('hostname') + '/dashboard/warehouse/inventory/reconciliation'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Warehouse Inventory Product Reconciliation",
    pageTitle: "Warehouse Inventory Product Reconciliation"
  }

  res.render('pages/warehouse_inventory/product_reconciliation', {
    scripts: scripts,
