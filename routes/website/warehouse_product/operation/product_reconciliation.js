...
/* GET login page. */
router.get('/reconciliation', urlAuth('warehouse product reconciliation'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'reconciliation',
      link: config.get('hostname') + '/dashboard/warehouse/product/reconciliation'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " -Warehouse Product Reconciliation",
    pageTitle: "Warehouse Product Reconciliation"
  }

  res.render('pages/warehouse_product/product_reconciliation', {
    scripts: scripts,
