...
/* GET Warehouse Transaction Discount Page. */
router.get('/manage-discount', urlAuth('warehouse transaction manage discount'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'discount',
      link: config.get('hostname') + '/dashboard/warehouse/transaction/manage-discount'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Warehouse Transaction",
    pageTitle: "Manage Warehouse Dicount",
    adminInfo: req.adminInfo,
  }

  res.render('pages/warehouse_transaction/discount', {
    scripts: scripts,
