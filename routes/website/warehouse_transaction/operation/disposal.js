...
/* GET Warehouse Transaction Disposal Page. */
router.get('/manage-disposal', urlAuth('warehouse transaction manage disposal'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'disposal',
      link: config.get('hostname') + '/dashboard/warehouse/transaction/manage-disposal'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Warehouse Transaction",
    pageTitle: "Manage Warehouse Disposal",
    adminInfo: req.adminInfo,
  }

  res.render('pages/warehouse_transaction/disposal', {
    scripts: scripts,
