...
/* GET Warehouse Transaction Receiving Page. */
router.get('/supplier-receive', urlAuth('warehouse transaction supplier receive'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'receive',
      link: config.get('hostname') + '/dashboard/warehouse/transaction/supplier-receive'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Warehouse Transaction",
    pageTitle: "Receive From Warehouse Supplier",
    adminInfo: req.adminInfo,
  }

  res.render('pages/warehouse_transaction/supplierReceiving', {
    scripts: scripts,
