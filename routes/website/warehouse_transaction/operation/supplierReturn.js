...
/* GET Warehouse Transaction Return Page. */
router.get('/supplier-return', urlAuth('warehouse transaction supplier return'), function (req, res, next) {
  const scripts = [
  ...
  const breadcumb = {
    ...
    sub: [{
      text: 'return',
      link: config.get('hostname') + '/dashboard/warehouse/transaction/supplier-return'
    }]
  }

  const data = {
    siteTitle: config.get('company').name + " - Warehouse Transaction",
    pageTitle: "Return To Warehouse Supplier",
    adminInfo: req.adminInfo,
  }

  res.render('pages/warehouse_transaction/returnToSupplier', {
    scripts: scripts,
